...
class CharRecall(_BasePrecisionRecall):
    r"""Calculates recall for binary and multiclass data.
    ...
    def update(self, output: Sequence[torch.Tensor]) -> None:
        ...
        y = y.tolist()
        common = Counter(y) & Counter(y_pred)
        true_positives = [sum(common.values())]
        actual_positives = [len(y)]
        true_positives = torch.Tensor(true_positives)
        actual_positives = torch.Tensor(actual_positives)
        true_positives = true_positives.to(dtype=torch.float64, device=self._device)
        actual_positives = actual_positives.to(dtype=torch.float64, device=self._device)
